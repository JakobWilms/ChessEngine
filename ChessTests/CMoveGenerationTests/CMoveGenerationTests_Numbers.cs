...
public class CMoveGenerationTestsNumbers
{
    ...
    {
        new CMove(CSquare.F2, CSquare.F3, MoveFlag.QuietMove, PieceType.WhitePawn, PieceType.Empty).Make(Board);
        new CMove(CSquare.D7, CSquare.D5, MoveFlag.DoublePawnPush, PieceType.BlackPawn, PieceType.Empty).Make(Board);
        new CMove(CSquare.E1, CSquare.F2, MoveFlag.QuietMove, PieceType.WhiteKing, PieceType.Empty).Make(Board);
        new CMove(CSquare.D5, CSquare.D4, MoveFlag.QuietMove, PieceType.BlackPawn, PieceType.Empty).Make(Board);
        Console.WriteLine(CAttackMap.ArrPawnAttacks[0][(int)CSquare.E3]);
        Console.WriteLine(CAttackMap.ArrPawnAttacks[0][(int)CSquare.E3] & Board.GetPawns(ColorType.Black));
        Assert.True(Board.Attacked(CSquare.E3, ColorType.Black));
        Assert.AreEqual(20, MoveGen_Root(1));
    }
    ...
    {
        Board = FenReader.ImportFen("r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - ");
    ...
    {
        Board = FenReader.ImportFen("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - ");
    ...
    {
        Board = FenReader.ImportFen("r3k2r/Pppp1ppp/1b3nbN/nP6/BBP1P3/q4N2/Pp1P2PP/R2Q1RK1 w kq - 0 1");
        new CMove(CSquare.D2, CSquare.D4, MoveFlag.DoublePawnPush, PieceType.WhitePawn, PieceType.Empty).Make(Board);
        new CMove(CSquare.A3, CSquare.A2, MoveFlag.Capture, PieceType.BlackQueen, PieceType.WhitePawn).Make(Board);
        Assert.AreEqual(41, CMoveGeneration.MoveGen(Board).Count);
    }
